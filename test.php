...
{
try{
    $sql = "INSERT INTO wk VALUES (0,2,'test','',:kd_komponen,:qty,:kegiatan)";
...
<body class="hold-transition dark-mode">
  ...
  <!-- show data tables kd_ring -->
   ...
   <input type="hidden" name="data[=<?php echo $ring['kd_ring']?>][kd_ring]">
  </div>
  ...
  <input type="hidden" name="data=[<?php echo $komponen['kd_komponen']?>][kd_komponen]">
    ...
    <input type="text" name="data=[<?php echo $komponen['kd_komponen']?>][qty]" class="form-control" autocomplete="off">
    <label class="col-sm-10">Kegiatan</label>
    <input type="text" name="data=[<?php echo $komponen['kd_komponen']?>][kegiatan]" class="form-control" autocomplete="off">
    <br>
    </div>
    <?php }?>
  </div>
